...
class OrderImportDetail extends Mapper implements \MVC\Domain\OrderImportDetailFinder {
    ...
    function __construct() {
		...
		$tblResource = "haokiet_resource";
		$tblOrderImport = "haokiet_order_import";
		$tblOrderImportDetail = "haokiet_order_import_detail";
		$tblSessionDetail = "haokiet_session_detail";
		$tblR2C = "haokiet_r2c";
		...
		$trackByExportStmt = sprintf("
			...
			from
				haokiet_session S inner join haokiet_session_detail SD on S.id = SD.idsession
			where
				SD.idcourse IN(select id_course from haokiet_r2c where id_resource=?) AND
