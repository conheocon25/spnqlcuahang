...
class CustomerLog extends Mapper implements \MVC\Domain\CustomerLogFinder {
    ...
    function __construct() {
        parent::__construct();
        $this->selectAllStmt = self::$PDO->prepare("select * from shopc_customer_log");
        $this->selectStmt = self::$PDO->prepare("select * from shopc_customer_log where id=?");
        $this->updateStmt = self::$PDO->prepare("update shopc_customer_log set id_customer=?, datetime=?, ticket1=?, ticket2=?, paid1=?, paid2=?, debt=?, paid1_remain=?, paid2_remain=? where id=?");
        $this->insertStmt = self::$PDO->prepare("insert into shopc_customer_log (id_customer, `datetime`, ticket1, ticket2, paid1, paid2, debt, paid1_remain, paid2_remain) values( ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$this->deleteStmt = self::$PDO->prepare("delete from shopc_customer_log where id=?");		
		
		$this->findByCustomerStmt 	= self::$PDO->prepare("select * from shopc_customer_log where id_customer=? order by datetime desc");
		$this->findByDateStmt 		= self::$PDO->prepare("			
			select * 
			from 				
				shopc_customer_log 	CL				
			where
			date(`datetime`)=?
		");
		...
		$this->findByDate1Stmt 		= self::$PDO->prepare("			
			...
			order by name
		");
		$this->findActiveStmt 		= self::$PDO->prepare("select * from shopc_customer_log where date(`datetime`)=? AND id_customer=?");
		$this->findPreStmt 			= self::$PDO->prepare("select * from shopc_customer_log where date(`datetime`) < date(?) AND id_customer=? ORDER BY `datetime` DESC LIMIT 1");
    } 
