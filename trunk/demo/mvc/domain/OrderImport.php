...
class OrderImport extends Object{
	...
	function getDetailAllPrint(){		
		$mOID = new \MVC\Mapper\OrderImportDetail();
		$Detail = $mOID->findBy(array($this->getId()));
		return $Detail;
	}
	...
	//DEFINE URL
	//-------------------------------------------------------------------------------
	function getFindResourceByBarcode(){return "/import/".$this->getIdSupplier()."/".$this->getId()."/barcode";}
	
	function getURLPrint(){return "/import/".$this->getIdSupplier()."/".$this->getId()."/print";}
	function getURLPrint1(){return "/import/".$this->getIdSupplier()."/".$this->getId()."/print/1";}
	
	function getURLUpdLoad(){return "/import/".$this->getIdSupplier()."/".$this->getId()."/upd/load";}
	function getURLUpdExe(){return "/import/".$this->getIdSupplier()."/".$this->getId()."/upd/exe";}
	
	function getURLDelLoad(){return "/import/".$this->getIdSupplier()."/".$this->getId()."/del/load";}
	function getURLDelExe(){return "/import/".$this->getIdSupplier()."/".$this->getId()."/del/exe";}
