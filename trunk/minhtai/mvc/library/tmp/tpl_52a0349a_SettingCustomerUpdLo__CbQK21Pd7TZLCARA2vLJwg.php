<?php 
function tpl_52a0349a_SettingCustomerUpdLo__CbQK21Pd7TZLCARA2vLJwg(PHPTAL $tpl, PHPTAL_Context $ctx) {
$_thistpl = $tpl ;
...
$_tmp_2 = $ctx->repeat ;
$_tmp_2->lst = new PHPTAL_RepeatController($ctx->Type)
 ;
$ctx = $tpl->pushContext() ;
foreach ($_tmp_2->lst as $ctx->lst): ;
if (null !== ($_tmp_3 = ($ctx->lst['Id']))):  ;
$_tmp_3 = ' value="'.phptal_escape($_tmp_3).'"' ;
else:  ;
$_tmp_3 = '' ;
endif ;
if ($ctx->Customer->getType()==$ctx->Type['Id']?true:false):  ;
...
<span><?php echo phptal_escape($ctx->lst['Name']); ?>
</span>
							</option><?php 
endforeach ;
$ctx = $tpl->popContext() ;
...
if (null !== ($_tmp_4 = ($ctx->path($ctx->Customer, 'getType')))):  ;
